...
function generateballs(ctx, stateMap) {
  ...
  stateMap.forEach(function (states, key_color) {
    ctx.beginPath();
    for (var i = 0; i < states.length; i++) {
      ...
      if (ballMap.has(newball.color)) {
        var ballObjs = ballMap.get(newball.color);
...
__$styleInject(".root{margin:0}canvas{background:#000}#left,#right{float:left}#right{margin-left:30;width:15rem}#score{font-size:1rem}", {});
...
function random_int_range(min, max) {
  if (max <= min && min !== 0 && max !== 0) throw new TypeError("max must be larger than min");
...
function generate_states() {
  ...
  for (var i = 1; i <= num; i++) {
    ...
    for (var j = 1; j <= num; j++) {
      ...
      var ballColor = colors[random_int_range(0, colors.length - 1)];
      var ballObj = {
        x: x,
        y: y,
        r: r,
      ...
      if (stateMap.has(ballColor)) {
        var ballObjs = stateMap.get(ballColor);
...
function start() {
  stats.begin();
  clear(ctx);
  ballMap.forEach(function (balls, key_color) {
    ctx.beginPath();
