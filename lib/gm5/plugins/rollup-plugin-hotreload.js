...
export function rph(options) {
  ...
  return {
    ...
    onwrite: function (opts) {
      ...
      createReadStream(join(__dirname, "./plugins/hotreload.js")).pipe(createWriteStream(join(__dirname, `./${rootDir}/hotreload.js`)));
...
export function multibundles(config, dirname) {
  if (!Array.isArray) {
